...
$(document).ready(function() {
	$('#test').click(function(e) {
		...
		$.soap({
			...
			appendMethodToURL: $('#appendMethodToURL').attr('checked'),
			SOAPAction: $('#SOAPAction').val(),
			soap12: $('#soap12').attr('checked'),
			async: $('#async').attr('checked'),
			...
			*/
			...
			noPrefix: $('#noPrefix').attr('checked'),
			elementName: $('#elementName').val(),

			enableLogging: $('#enableLogging').attr('checked'),
...
function writeSoapCall() {
	...
	if ($('#appendMethodToURL').attr('checked') == false) { options.push('  appendMethodToURL: false'); }
	if ($('#SOAPAction').val()) { options.push('  SOAPAction: "' + $('#SOAPAction').val() + '"'); }
	if ($('#soap12').attr('checked') == true) { options.push('  soap12: true'); }
	if ($('#async').attr('checked') == false) { options.push('  async: false'); }
	...
	if ($('#noPrefix').attr('checked') == true) { options.push('  noPrefix: true'); }
	if ($('#elementName').val()) { options.push('  elementName: "' + $('#elementName').val() + '"'); }
	if ($('#enableLogging').attr('checked') == true) { options.push('  enableLogging: true'); }
	if ($('#enableWSS').attr('checked')) {
		options.push('  wss: {\n    username: "' + $('#wssUsername').val() + '"');
	...
	callout.push('});');
	callout.push('','url used: ' + ($('#appendMethodToURL').attr('checked') ? $('#url').val() + $('#method').val() : $('#url').val()))
	return callout.join('\n');
}
